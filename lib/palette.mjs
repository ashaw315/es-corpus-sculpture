...
// Per-node color variation. Each sentence gets a unique hue/lightness
// offset within its style_mode family — ±15° hue, ±8% lightness — seeded
// deterministically by neon_id so reload and palette switches keep the
// same internal pattern across the corpus.
function hashId(id) {
  // FNV-1a 32-bit. Cheap, well-distributed, no deps. Returned as unsigned.
  let h = 0x811c9dc5
...
export function nodeColor(node, palette) {
  const base = palette?.colors?.[node.style_mode]
  if (!base) return DEFAULT_COLOR
  const m = base.match(/hsl\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*\)/)
  if (!m) return base
  const h0 = Number(m[1]), s0 = Number(m[2]), l0 = Number(m[3])
  // Two independent offsets from the same hash — top 16 bits → hue,
  // bottom 16 → lightness — both mapped into [-1, 1].
  const hash = hashId(String(node.id ?? node.neon_id ?? ''))
  const hueT = ((hash >>> 16) / 0xffff) * 2 - 1
  const lightT = ((hash & 0xffff) / 0xffff) * 2 - 1
  const h = (h0 + hueT * 15 + 360) % 360
  const l = Math.max(10, Math.min(90, l0 + lightT * 8))
  return `hsl(${Math.round(h)}, ${s0}%, ${Math.round(l)}%)`
}
